...
class Guangdiu
{
    ...
    private function check_new($url, $cate_key = null, $type = 1)
    {
        ...
        if (empty($guangdiuLastId)) {
            //没有采集过
            log_debug('没有采集过');
            return $this->get_list_info($url, '1', $type, $cate_key);
        } else {
            //查询当前最新商品id
            ...
            if ($last_id <= $guangdiuLastId) {
                ...
                log_debug('更新缓存中的guangdiu_last_id');
                $cache_name = $cate_key . "_last_id";
                S($cache_name, $guangdiu_last_id);
                return $this->get_list_info($url, $guangdiuLastId, $type, $cate_key);
            }
     ...
     */
    private function get_list_info($url, $guangdiuLastId, $type = '1', $cate_key)
    {
